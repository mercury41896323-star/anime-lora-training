...
In the 2.5D-first architecture, the profile is also the common source registry. `source_assets` stores videos and external images, `definition_2p5d` points to the active 2.5D Definition, `domain_models` stores motion/camera/background/lighting trainer outputs, and `learning_strategy` is `2p5d_base_lora_completion`.
